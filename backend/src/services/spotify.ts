...
import { getSpotifyConfig, updateSpotifyConfig } from '../utils/db';
...
export async function exchangeAuthCode(code: string, codeVerifier: string): Promise<boolean> {
  ...
  if (!response.ok) {
    logError('Errore scambio auth code', { status: response.status });
    return false;
  }
...
export async function getPlaylistTrackCount(): Promise<number> {
  try {
    const accessToken = await refreshAccessToken();
    if (!accessToken) return 0;
    const config = await getSpotifyConfig();
    if (!config?.playlistId) return 0;
    const response = await fetch(
      `https://api.spotify.com/v1/playlists/${config.playlistId}?fields=tracks(total)`,
      { headers: { Authorization: `Bearer ${accessToken}` } }
    );
    if (!response.ok) return 0;
    const data = (await response.json()) as { tracks?: { total?: number } };
...
export function generateYoutubeSearchUrl(title: string, artist: string): string {
  ...
  return `https://youtube.com/results?search_query=${q}`;
}

export function generateSoundcloudSearchUrl(title: string, artist: string): string {
  const q = encodeURIComponent(`${artist} ${title}`);
  return `https://soundcloud.com/search/sounds?q=${q}`;
}
